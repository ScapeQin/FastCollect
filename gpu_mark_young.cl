...
void enqueue2 (__global int* queue, int* tail, int* head, int data)
{
      if (*head == -1)
      {
        *tail = data;
...
inline int dequeue2 (__global int* queue, int* tail, int* head)
{
...
__kernel void mark_phase (__global Node* nodes, __global int* adjacent_vertices, __global int* roots, __global int* queue, int n_root_nodes, __global int* total_youngs, __global int* young_queue, unsigned long _reserved, __global int* queue2, int total_nodes)
{
  ...
  __local int* local_queue = _local_queue + (local_id<<max_local_queue_size_power);
	int young_queue_head = -1;
	int young_queue_tail = -1;
	int young_size = 0;
	int work_steal_length = total_nodes/n_root_nodes;
  int work_steal_start = id*work_steal_length + 1;
  //int work_steal_start2 = work_steal_start;
    ...
    for (i = nodes[j].first_index; i < nodes[j].last_index; i++)
    {
      ...
      if (nodes [vertex].marked == 0)
      {
        if (atomic_cmpxchg (&nodes [vertex].marked, 0, 1) == 0)
        {
          ...
          if (nodes[vertex].address < _reserved)
          {
            enqueue2 (queue2, &young_queue_tail, &young_queue_head, vertex);
            young_size ++;
          }
  ...
  if (young_size != 0)
  {
    int i = atomic_add (total_youngs, young_size);

    for (int j = i; j < young_size; j++)
    {
      young_queue[j] = dequeue2(queue2, &young_queue_tail, &young_queue_head);
    }
  }
}
